...
class HyprConfGUI(Window):
    def __init__(self, show_lock_checkbox: bool, show_idle_checkbox: bool, **kwargs):
        super().__init__(
            title="SyndraShell Settings",
            name="syndrashell-settings-window",
            size=(640, 640),
            **kwargs,
        )
    ...
    def create_key_bindings_tab(self):
        ...
        bindings = [
            ...
            ("Message", "prefix_syndramsg", "suffix_syndramsg"),
    ...
    def create_system_tab(self):
        ...
        self.auto_append_switch = Gtk.Switch(
            ...
            tooltip_text="Automatically append SyndraShell source string to hyprland.conf",
        )
        ...
        monitor_label = Label(
            label="Show SyndraShell on monitors:", h_align="start", v_align="center"
        )
        ...
        if self.show_lock_checkbox:
            ...
            self.lock_switch = Gtk.Switch(
                tooltip_text="Replace Hyprlock configuration with SyndraShell's custom config"
            )
        ...
        if self.show_idle_checkbox:
            ...
            self.idle_switch = Gtk.Switch(
                tooltip_text="Replace Hypridle configuration with SyndraShell's custom config"
            )
    ...
    def create_about_tab(self):
        ...
        repo_link = Label(
            markup='<a href="https://github.com/Fud0o0/SyndraShell">https://github.com/Fud0o0/SyndraShell</a>'
        )
        repo_box.add(repo_label)
        repo_box.add(repo_link)
        vbox.add(repo_box)

        # Ko-Fi support link removed (not the same author as SyndraShell)
    ...
    def on_accept(self, widget):
        ...
        def _apply_and_reload_task_thread(user_data):
            ...
            print(f"{time.time():.4f}: Initiating SyndraShell restart using Popen...")
            ...
            print(f"{time.time():.4f}: SyndraShell restart commands issued via Popen.")
            end_time = time.time()
            print(
