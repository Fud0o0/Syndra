    ...
    if (
        ...
        if not os.path.exists(current_wall):
            example_wallpaper_path = os.path.expanduser(
                f"~/.config/{APP_NAME_CAP}/assets/wallpapers/default.png"
            )
...
$syndraMessage = notify-send "SyndraShell" "FIRE IN THE HOLE‼️🗣️🔥🕳️" -i "{home}/.config/{APP_NAME_CAP}/assets/syndra.png" -A "🗣️" -A "🔥" -A "🕳️" -a "Source Code"
...
bind = {get_bind_var("prefix_syndramsg")}, {get_bind_var("suffix_syndramsg")}, exec, $syndraMessage # Message
