...
# Full spectrum security installation (Red + Blue)

set -e

echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║                   SYNDRA - PURPLE TEAM                        ║"
echo "║          Full Spectrum Security Environment                   ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
...
echo ""
echo "🟣 Installing Purple Team tools and configuration..."
...
sudo pacman -S --noconfirm wireshark-qt snort suricata aide rkhunter \
    ...
    ossec-server tripwire htop iotop nethogs iftop glances \
...
pip install --user impacket crackmapexec bloodhound pwntools \
    ...
    yara-python sigma-cli splunk-sdk elasticsearch loguru pandas numpy
...
yay -S --noconfirm sublister maltego feroxbuster nuclei subfinder \
    amass httpx waybackurls gau wazuh-agent osquery zeek elastalert
...
echo "📝 Applying Purple Team configuration..."
...
# Set Purple Team color scheme
sed -i 's/--primary: #.*/--primary: #b366ff;/' ~/.config/waybar/style.css 2>/dev/null || true
...
echo ""
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║             ✅ PURPLE TEAM INSTALLATION COMPLETE!             ║"
echo "╠═══════════════════════════════════════════════════════════════╣"
